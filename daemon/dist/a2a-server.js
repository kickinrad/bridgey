...
import { saveMessage, getMessages, getAgents, saveAuditEntry, getAuditLog, getOrCreateConversation } from './db.js';
 ...
 */
export function a2aRoutes(fastify, config) {
    ...
    fastify.addHook('onResponse', async (req, reply) => {
        ...
        if (skipPaths.includes(pathOnly))
            return;
        // Determine auth_type
        let auth_type = 'none';
    ...
    fastify.post('/', async (req, reply) => {
        ...
        switch (method) {
            case 'message/send': {
                ...
                const response = await requestQueue.enqueue(agentName, () => executePrompt(messageText, config.workspace, config.max_turns));
                // Save to DB
